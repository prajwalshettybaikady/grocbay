...
class HelperFunction
{
    ...
    public function decodeJson($data, $order)
    {
        ...
        foreach ($row as $res) {
            ...
            $this->sendSms("+971" . $res['mobileNo'], $message);
        }
// sendOrderMessage($mobile);
    }
    public function sendOrderMessageCustomer($id, $order)
    {
        $connection = $this->em->getConnection();
...
// Sending email
    ...
    public function sendStatus($mobile, $id, $mode)
    {
        ...
        if ($mode == 'delivered') {
            ...
            $message = 'Your order ' . $id . ' is delivered.Thank you for choosing us!';
        ...
        if ($mode == 'onway') {
            ...
            $message = 'Your order ' . $id . ' is on way!';
        ...
        if ($mode == 'out') {
            ...
            $message = 'Your order is out for delivery. Will be delivered by ' . $id[0] . '(' . $id[1] . ').';
...
//Edit below parameters with Your Details
        $user = "Spikesouq"; //your account username
        $password = "Spi9060"; //your account password
        // $mobilenumbers="+971589174232";  //For multiple mobile numbers use comma as separator
        // $message = "test messgae"; //enter Your Message
        $senderid = $sender; //Your senderid
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,
            "User=$user&passwd=$password&mobilenumber=+971$mobile&message=$message&sid=$senderid&mtype=$messagetype");
...
//Edit below parameters with Your Details
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_POSTFIELDS,
            "User=$user&passwd=$password&mobilenumber=$number&message=$message&sid=$senderid&mtype=$messagetype");
        $ret = curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
...
//If you are behind proxy then please uncomment below line and provide your proxy ip with port.
    ...
    public function sendGCM($message, $id, $mode, $orderid)
    {
        $response = $this->getGlobal();
    ...
    public function sendGCMultiple($message, $id, $link, $mode, $ref, $nid)
    {
        $response = $this->getGlobal();
        ...
        $this->sendToFireBase($accessKey, $message, $id, $title, $link, $mode, $ref, $nid);
    }

    public function sendToFireBase($accessKey, $message, $id, $title, $link, $mode, $ref, $nid)
    {
