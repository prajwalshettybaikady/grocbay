...
{
     $connection = $em->getConnection();
$statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id in(select menu_item_id from price_variation where stock is not null or stock > 1) order by a.id desc");
...
{
        $connection = $em->getConnection();
$statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id in(select menu_item_id from price_variation where  stock is not null) order by a.id desc limit 5 OFFSET $rows");
...
else
{
  ...
  public function getDiscountAction(Request $request)
 {
    $em = $this->getDoctrine()->getManager();
     $connection = $em->getConnection();
$statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and  FIND_IN_SET('52', a.category) and a.id in(select menu_item_id from price_variation where stock is not null or stock > 1) order by a.id desc limit 5 OFFSET 0");
...
$out=$em->getRepository('AppBundle:User')->findBy(array('mobileNumber' =>$mobile,'password' =>$pass,'userType'=>'Delivery'));
  ...
  $connection = $em->getConnection();
     $statement = $connection->prepare("SELECT * from brands where parent_id is null order by id desc");
...
foreach ($menus as $res)
{
    $priceVariation=$this->existAction("SELECT * FROM `menu_item` where brand='".$res['id']."'");
if($priceVariation==0)
{
 ...
 public function bannerAction(Request $request)
     ...
     public function resturantAction(Request $request)
    {
       $em = $this->getDoctrine()->getManager();
        ...
        $menus=$em->getRepository('AppBundle:Restaurants')->findById('999');
...
public function getMenuItemAction(Request $request) 
    {
       ...
       // $ref=$em->getRepository('AppBundle:MenuItem')->findBy(array('brand'=>$id));
        $connection = $em->getConnection();
$statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and find_in_set('$id',a.category) and a.id in(select menu_item_id from price_variation where stock is not null or stock > 2) limit 5 OFFSET $start");
...
public function getMenuItemByBrandAction(Request $request)
    {
       ...
       // $ref=$em->getRepository('AppBundle:MenuItem')->findBy(array('brand'=>$id));
        $connection = $em->getConnection();
$statement = $connection->prepare("SELECT * FROM `menu_item` where brand='$id' and id in(select menu_item_id from price_variation where stock is not null or stock > 1) and isActive='1'  limit 5 OFFSET $start");
...
// }
     ...
     // $helper->ordersms("9632618670",$customerOrder9);
  ...
  $connection = $em->getConnection();
        ...
        if($mode=='getAll')
            {
     $statement = $connection->prepare("SELECT * from sub_category where parent_id is null order by id asc");
...
foreach ($menus as $res)
{
    $priceVariation=$this->existAction("SELECT * FROM `sub_category` where parent_id='".$res['id']."'");
if($priceVariation==0)
{
...
{
$ratings[]=array('id'=>$res['id'],'category_name'=>$res['categoryName'],'icon_image'=>$res['iconImage'],'SubCategory'=>$this->getSubcategoryAction($res['id']));
}
}
            }
            else{
           $statement = $connection->prepare("SELECT * from sub_category where parent_id is null order by id desc");
...
foreach ($menus as $res)
{ 
$menu[]=array('id'=>$res['id'],'category_name'=>$res['categoryName'],'icon_image'=>$res['iconImage']);
}
return $menu;
    }
...
$em = $this->getDoctrine()->getManager();
$connection = $em->getConnection();
$sql = $connection->prepare("SELECT * FROM `membership` where parent_id='0'");
$sql->execute();
$row = $sql->fetchAll();
foreach ($row as $res){
  $rows[]=array("name" => $res['id']);


}

  return array("data"=>$ows);
}
public function getmembershipTypesAction($id)
{
  $em = $this->getDoctrine()->getManager();
$connection = $em->getConnection();
$sql = $connection->prepare("SELECT * FROM `membership` where parent_id='$id'");
$sql->execute();
$row = $sql->fetchAll();
foreach ($row as $res){
