...
//getapi key
            ...
            $postvar = 'branch=' . $branch . '&userId=' . $userid . '&note=' . $note . '&walletType=' . $walletType . '&walletBalance=' . $walletBalance . '&apiKey=' . $apiKey . '&restId=' . $restId . '&addressId=' . $addressid . '&orderType=' . $orderType . '&paymentType=' . $payment . '&promocode=' . $promocode . '&fix_time=' . $fix_time . '&channel=Admin&fixdate=' . $fixDate . '&membership_active=' . $ra['membership'] . '&membership=0&orderRef='.$orderRef;
...
//function to create logs
    ...
    public function thermalPrintAction(Request $request)
    {
        ...
        return $this->render('AppBundle:Admin:Orders/thermal.html.twig', array(
            'orders' => $mes,
            'merged_orders' => $this->getMergedOrder($id),
            'merged_items' => $this->getMergedItems($id),
            'merged_tax'=>$this->getMergedTax($id),
    ...
    public function getMergedOrder($id)
    {
 $helper = $this->container->get('function_helper');
        $em = $this->getDoctrine()->getManager();
        $connection = $em->getConnection();
        $ref=$this->getRef($id);
                $statement = $connection->prepare("SELECT a.*,b.mobileNo,b.email,b.gst FROM `customer_order` as a join customer as b on a.customer_d=b.id where a.id='$ref'");
        $statement->execute();
       return $mes = $statement->fetchAll();
    }
    public function getMergedTax($id)
    {
 $helper = $this->container->get('function_helper');
        $em = $this->getDoctrine()->getManager();
        $connection = $em->getConnection();
        $ref=$this->getRef($id);
        $statements = $connection->prepare("SELECT tax,sum(subTotal) as subTotal FROM `customer_order_items` where order_d='$ref' GROUP by tax order by tax asc");
        $statements->execute();
     return   $items = $statements->fetchAll();
    }
    public function getMergedItems($id)
    {
 $helper = $this->container->get('function_helper');
        $em = $this->getDoctrine()->getManager();
        $connection = $em->getConnection();
        $ref=$this->getRef($id);
              $statements = $connection->prepare("SELECT * FROM `customer_order_items` where order_d='$ref' order by tax ASC");
        $statements->execute();
     return   $items = $statements->fetchAll();
    }
...
// $time = date('Y-m-d H:i:s');
        ...
        $sql->execute();
        $location = $sql->fetchAll();
        // return new JsonResponse($this->customerOrderMerged($customerOrder->getId(),$customerOrder->getRef()));
            return $this->render('AppBundle:Admin:Orders/orderForm.html.twig', array(
            'order' => $customerOrder,
            'merged_order' => $this->customerOrderMerged($customerOrder->getId(),$customerOrder->getRef()),
            'merged_main_order' => $this->customerMainOrderMerged($customerOrder->getId(),$customerOrder->getRef()),
            'form' => $form->createView(),
            'myExtend' => $myExtend,
            'restaurant' => $restaurant,
            'dname' => $name,
...
public function customerOrderMerged($id,$ref)
{
    if($ref==0)
    {
        return array();
...
$customerOrder = new CustomerOrderItems();
                $customerOrder = $em->getRepository('AppBundle:CustomerOrderItems')->findByCustomerOrder($ref);          
            return $customerOrder;

    }
}

public function customerMainOrderMerged($id,$ref)
{
    if($ref==0)
    {
        return array();
...
$customerOrder = new CustomerOrder();
                $customerOrder = $em->getRepository('AppBundle:CustomerOrder')->findById($ref);          
            return $customerOrder;
