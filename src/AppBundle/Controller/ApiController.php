...
public function search_value($branch,$search_value,$user_id,$status){

        $em = $this->getDoctrine()->getManager();
        $connection = $em->getConnection();
        $time=date('d-m-Y'). ' at '. date('h-i A');

        if(strlen($search_value)>5){
            $sql = "INSERT INTO search_history 
                    (branch, search_value, user_id, time,status) 
                    VALUES (:branch, :search_value, :user_id, :time, :status)";
    ...
    public function searchProductAllAction(Request $request)
    {
        ...
        $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and 1 and a.id in(select menu_item_id from price_variation where status='0' and $query)  and  a.itemName  LIKE '%" . $searchProduct . "%' and a.branch='$branch' $oquery order by a.regularPrice desc Limit 10");
        ...
        if (count($menus) == 0) {
            $data['data'] = array();
            $this->search_value($branch,$searchProduct,$apiKey,0);
        } else {
            $data['data'] = $menu;
                        $this->search_value($branch,$searchProduct,$apiKey,1);
    ...
    public function singleProductAction(Request $request)
    {
        ...
        $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id='$id' order by a.regularPrice desc");
        ...
        foreach ($row as $res) {
            ...
            if ($bannertype == '2') {
                ...
                if ($parentId == '') {
                    ...
                    $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and  find_in_set('$dataId',REPLACE(a.category, ', ', ',')) and a.id in(select menu_item_id from price_variation where $query) $oquery order by a.regularPrice desc");
                ...
                $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id in(select menu_item_id from price_variation where $query) and a.id in($dataId) $oquery order by a.regularPrice desc");
    ...
    public function getMenuItemByBrandAction(Request $request)
    {
        ...
        $connection = $em->getConnection();
        $statement = $connection->prepare("SELECT a.*,b.categoryName as brandss FROM `menu_item` as a join brands  as b on a.brand=b.id where a.brand='$id' and a.id in(select menu_item_id from price_variation where $query and status='0') $oquery and a.isActive='1' order by a.regularPrice desc limit 10 OFFSET $start");
...
// $ref=$em->getRepository('AppBundle:MenuItem')->findBy(array('brand'=>$id));
        $connection = $em->getConnection();
        $statement = $connection->prepare("SELECT a.*,b.categoryName as brandss FROM `menu_item` as a join brands  as b on a.brand=b.id where a.id in($id) and a.id in(select menu_item_id from price_variation where $query and status='0') $oquery and a.isActive='1' order by a.regularPrice desc");
        $statement->execute();
        $menus = $statement->fetchAll();
        if (count($menus) == 0) {
            $menu = array();
    ...
    public function getMenuItemAction(Request $request)
    {
        ...
        $connection = $em->getConnection();
        if ($type == 0) {
            $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and find_in_set('$id',REPLACE(a.category, ', ', ',')) and a.id in(select menu_item_id from price_variation where  $query and status='0') and a.branch='$branch' $oquery order by a.regularPrice desc limit 10 OFFSET $start");
        } else {
            ...
            $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.category in($cate) and a.id in(select menu_item_id from price_variation where  $query and status='0')  and a.branch='$branch' $oquery order by a.regularPrice desc limit 10 OFFSET $start");
        }
    ...
    public function getDiscountAction(Request $request)
    {
        ...
        if ($rows == 0) {
            ...
            $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id in(select menu_item_id from price_variation where $query and status='0') and a.id in($resdata) $oquery and a.branch='$branch' order by a.regularPrice desc limit 100 OFFSET 0");
            ...
            $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id in(select menu_item_id from price_variation where  $query and status='0') and 1 and a.id in($resdata) $oquery and a.branch='$branch' order by a.regularPrice desc limit 5 OFFSET 0");
    ...
    public function getRecentProductsAction(Request $request)
    {
        ...
        $resdata = $ss['category'];
        $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.category in($resdata) and a.id in(select menu_item_id from price_variation where  $query and status='0') and a.id!='$id' $oquery  and a.branch='$branch' order by a.regularPrice desc limit 10 ");
        $statement->execute();
        $menus = $statement->fetchAll();
        $menu = array();
        foreach ($menus as $res) {
    ...
    public function getFeaturedAction(Request $request)
    {
        ...
        if ($rows == 0) {
            ...
            $connection = $em->getConnection();
            $statement = $connection->prepare("SELECT a.*,b.categoryName FROM `menu_item` as a join brands as b on a.brand=b.id where a.isActive='1' and a.id in(select menu_item_id from price_variation where  $query and status='0') and a.id in($resdata) $oquery order by a.regularPrice desc limit 5 OFFSET 0");
    ...
    {   
        ...
        $data = 0;
        if ($type == 0) {
            $statement = $connection->prepare("SELECT sum(stock) as stock FROM `price_variation` where menu_item_id='$id' and status=0");
            $statement->execute(); 
            ...
            if($data == '')
                $data = 0;
            $em->getConnection()->exec("update menu_item set regularPrice='$data' where id='$id'");
            return $data;
        } else {
    ...
    public function deletevariationimagesAction($id)
    {
        $em = $this->getDoctrine()->getManager();
        $connection = $em->getConnection();

        $res = $em->getConnection()->exec("DELETE FROM `variation_files` WHERE id='$id'");
