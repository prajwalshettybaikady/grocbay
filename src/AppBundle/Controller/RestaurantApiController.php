...
// return $customerOrder;
    ...
    public function getusername($id)
    {
        ...
        public function updatedMergedOrder($orderRef,$id)
    {
        $em = $this->getDoctrine()->getManager();
        $connection = $em->getConnection();
                    $em->getConnection()->exec("update customer_order set ref='$id',is_merged=1 where id='$orderRef'");
                    return 0;
    ...
    public function makeOrderAction(Request $request)
    {
        ...
        $customerOrder->setRef($orderRef);
        $customerOrder->setIsMerged($merged);
        $this->updatedMergedOrder($orderRef,$id);
                ...
                foreach ($items as $item) {
                    ...
                    if ($helper->testNull($item['productId'])) {
                        ...
                        if (!($menuItem instanceof MenuItem)) {
                            ...
                            $customerOrderItem->setReturnTime($menuItem->getDuration());
                            $customerOrderItem->setProductType($menuItem->getProductType());
                        ...
                        foreach ($datas as $d) {
                            ...
                            foreach ($finalresponse as $fi) {
                                ...
                                if ($next == 0 || $next == null) {
                                    ...
                                    //$enddifference = strtotime(date('h:i A')) - strtotime($To);
                                    $difference = strtotime(date('h:i A')) - strtotime($To);

                                    $timestamp = strtotime($now);
                                    $timestamp_one_hour_later = $timestamp + $seconds; // 3600 sec. = 1 hour

                                    // Formats the timestamp to HH:MM => outputs 11:09.

                                    $difference = $timestamp_one_hour_later - strtotime($fi['start']);

                                }

                                if ($next == '999') {
                                    if ($date == date('d-m-Y') && $difference < 0) {
...
$em->getConnection()->exec("update customer_order set total_price=0 where 1 and id='$order'");
    ...
    public function getItemsdata($id)
    {
        ...
        $res = $statement->fetchAll();
        foreach ($res as $resp) {
            $data[] = array("tax" => $resp['tax'],"id" => $resp['id'], "order_d" => $resp['order_d'], "itemId" => $resp['itemId'], "itemName" => $resp['itemName'].'-'.$resp['priceVariavtion'], "priceVariavtion" => $resp['priceVariavtion'], "price" =>  round($resp['price'],2), "quantity" => $resp['quantity'], "actualAmount" =>  round($resp['actualAmount'],2), "discount" =>  round($resp['discount'],2), "subTotal" =>  round($resp['subTotal'],2), "isTray" => $resp['isTray'], "trayQty" => $resp['trayQty'], "loyalty" => $resp['loyalty'], "image" => $this->getItemImage($resp['itemId']), "menuid" => $this->getMenuid($resp['itemId']), "barcode" => $this->getBarcode($resp['itemId']),'return_time'=>$resp['return_time']);
        }
        return $data;
...
// $filename   = uniqid() . "_" . time(); // 5dab1961e93a7_1571494241
        // $extension  = pathinfo( $_FILES["attachment"]["name"], PATHINFO_EXTENSION ); // jpg
        // $basename   = $filename . '.' . $extension; // 5dab1961e93a7_1571494241.jpg
        // $source       = $_FILES["attachment"]["tmp_name"];
        // $destionation = "../uploads/" . $basename;
        // /* move the file */
        // move_uploaded_file($_FILES["attachment"]["tmp_name"],$destionation );
        $priority = $request->get('priority');
        $sqln = $connection->prepare("SELECT * from ticket");
        $sqln->execute();
        $rown = $sqln->fetchAll();
        $ref = count($rown) + 10000;
        $res = $em->getConnection()->exec("INSERT INTO `ticket`(`ref`, `raisedBy`, `parent`, `subject`, `raisingDate`, `completedDate`, `type`, `priority`, `message`, `attachment`, `remarks`, `status`, `ticketStatus`,`callback`,`attachment_type`,`branch`) VALUES ('$ref','$id','$from','$subject','$date','','$type','High','$message','noimg.png','','Pending','Open','$callback','1',$branch)");
    ...
    public function attachment($id)
    {
        ...
        $uploads_dir = '../uploads/ticket';
        $targetDir = '../uploads/ticket/';
            ...
            foreach ($_FILES['image']['name'] as $key => $val) {
                ...
                // if (in_array($fileType, $allowTypes)) {
                    ...
                    if (move_uploaded_file($_FILES["image"]["tmp_name"][$key], $targetFilePath)) {
                        ...
                        $res = $em->getConnection()->exec("INSERT INTO `attachment`(`ref`, `file`) VALUES ('$id','$fileName')");
    ...
    public function getTicketAction(Request $request)
    {
        ...
        $connection = $em->getConnection();
        $id = $request->get('id');
        $sql = $connection->prepare("SELECT * from ticket where raisedBy='$id'");
        $sql->execute();
        $row = $sql->fetchAll();
        if (count($row) == '0') {
            return array();
