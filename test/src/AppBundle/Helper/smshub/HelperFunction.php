...
class HelperFunction
{
    ...
    {
        $message = "<#> ALMADINA : Your code is #" . $otp."  ".$signature;
    ...
    public function decodeJson($data, $order, $id)
    {
        ...
        foreach ($row as $res) {
          ...
          $message=" Dear " . $res['username'] . " your #" . $order->getId() . " has sucessfully placed with Al Madina Hypermarket LLC Abu Dhabi and is processing now.";
            $datas = array(
...
// Sending email
    ...
    public function sendStatus($mobile, $id, $mode)
    {
        ...
        if ($mode == 'onway') {
            ...
            $this->sendSms($mobile, "Your order ".$id." is on the way for delivery");
        }
        ...
        if ($mode == 'out') {
            ...
            $this->sendSms($mobile, "Your order order_no is assigned to " . $id[0] . " (" . $id[1] . ").");
        }
        if ($mode == 'pickup') {
            $datas = array(
                "mobiles" => "91" . $mobile,
            ...
            $this->sendSms($mobile, 'Your order #'.$id[0].' is ready for pickup from '.$id[1].'.');
        }
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
    ...
    public function sendSmsOtp($mobile = null, $message = null, $otp = null)
    {
        ...
        $user = "AlMadina.tr"; //your account username
        $apiKey = "hbC42vgQ"; //your account password
        $senderid = 'ALMADINAHYP'; //Your senderid
        
        $url = "http://portal.smshub.live/API/SendSMS";
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS,
            "username=$user&apiId=$apiKey&json=True&destination=971$mobile&source=$senderid&text=$message");
    ...
    public function sendSms($number, $message)
    {
        ...
        curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
        curl_setopt($ch, CURLOPT_POSTFIELDS,
            "username=$user&apiId=$apiKey&json=True&destination=971$number&source=$senderid&text=$message");
        $ret = curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
