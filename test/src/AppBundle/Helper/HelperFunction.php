...
class HelperFunction {
  ...
  /**
    ...
    public function sendOtp($mobile,$otp,$signature){
        ...
        public function sendOtpSms($mobile,$otp,$signature=null){
        $message="Your OTP is #".$otp.". Please do not share this with anyone.";
        $this->sendSmsOtp($mobile,$message,$otp,$signature);
    }
...
$message.='<img src="https://freshcompany.in/email.jpg" style="width:100%;">';
 ...
 mail($to, $subject, $message, $headers);
         ...
         public function sendSmsOtp($mobile=null, $message=null,$otp=null,$signature){
             $response=$this->getGlobal();
 $sender=$response['sms'];
...
$key=trim($template['otp']);
        if($mobile!=null && $message!=null):
            $message=urlencode($message);
                  // $message=urlencode($message);
            $param=json_encode(array("OTP"=>$otp,"KEY"=>$signature));
