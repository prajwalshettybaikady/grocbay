...
mail($to, $subject, $message, $headers);
    ...
    public function sendOtp($mobile, $otp,$signature)
    { 
        $message = "Your OTP is #" . $otp . ". Please do not share this with anyone. -- PVC Bazar";
        $this->sendSmsOtp($mobile, $message, $otp,$signature);
    }
    public function sendOtpSms($mobile, $otp,$signature)
    {
        $message = "Your OTP is #" . $otp . ". Please do not share this with anyone. -- PVC Bazar";
        $this->sendSmsOtp($mobile, $message, $otp,$signature);
    }
    ...
    public function decodeJson($data, $order, $id)
    {
        ...
        foreach ($row as $res) {
            $message = "Thank you ".$res['username']." for purchasing with ".$restaurantName.". Your order #".$order->getId()." is now processing -- PVC Bazar";
            
            $datas = array(
                "numbers" => "91" . $res['mobileNo'],
...
// Sending email
    ...
    public function sendStatus($mobile, $id, $mode)
    {
        ...
        if ($mode == 'delivered') {
            $datas = array(
                "numbers" => "91" . $mobile,
                "message" => trim('Your order '.$id.' is delivered.Thank you for choosing us -- PVC Bazar'),
                "order" => $id,
                "sender" => $sender,
            );
        ...
        if ($mode == 'onway') {
            $datas = array(
                "numbers" => "91" . $mobile,
                "message" => trim('Your order '.$id.' is on the way -- PVC Bazar'),
                "order" => $id,
                "sender" => $sender,
            );
        ...
        if ($mode == 'out') {
            $datas = array(
                "numbers" => "91" . $mobile,
                "message" => trim('Your order is out for delivery. Will be delivered by '.$id[0].' ('.$id[1].'). -- PVC Bazar'),
                "name" => $id[0] . '(' . $id[1] . ') .' . $id[2],
                "sender" => $sender,
            );
        ...
        if ($mode == 'cancelled') {
            $datas = array(
                "numbers" => "91" . $mobile,
                "message" => trim('Your order ('.$id.') has been canceled. -- PVC Bazar'),
                "order" => $id,
                "sender" => $sender,
            );
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />';
    ...
    public function sendSmsOtp($mobile = null, $message = null, $otp = null)
    {
        ...
        if ($mobile != null && $message != null):
            ...
            $data = array('apikey' => $apiKey, 'numbers' => $mobile, "sender" => $sender, "message" => $message); // for live mode

            //$data = array('apikey' => $apiKey, 'numbers' => $mobile, "sender" => $sender, "message" => $message, "test" => true); // for test mode
...
$response=$this->getGlobals($branch);
 ...
 // $title=$response['restaurantName'];
  ...
  $countryCode=$response['countryCode'];
    ...
    // public function sendGlobalSms($data)
    // {
    ...
    //     $smsgateway = $response['smsgateway'];

    //     if($smsgateway == "msg91"){

    //       $url = "http://api.msg91.com/api/v5/flow/"; // msg91
    //       $curl = curl_init();
    ...
    //     }else if($smsgateway == "textlocal"){ 
          
    //       $url = "https://api.textlocal.in/send/"; // textlocal

    //       // Start testing block
         
    //       //$message = urlencode('Your password is 123456 -- PVC Bazar');
    //       $data["test"] = true;

    //       // End testing block

    //       $data["numbers"] = $data["mobiles"];
    //       $data["apikey"] = $apiKey;
    ...
    //       // Process your response here (testing)
	  //       echo $response;

    //     }else if($smsgateway == "kaleyra"){
    ...
    // public function sendGlobalSmsOtp($mobile = null, $message = null, $otp = null)
    // {
    //     $response = $this->getGlobal();
    //     $sender = $response['sms'];
    //     $apiKey = $response['apiKey'];
    ...
    //         // $message=urlencode($message);
    //         $param = json_encode(array("OTP" => $otp)); 
    //         $apiRoute = "https://api.msg91.com/api/v5/otp?authkey=" . $apiKey . "&mobile=91" . $mobile . "&template_id=" . $key . "&sender=" . $sender . "&extra_param=" . $param . "&unicode=1"; 
    //         // http://sms.chotaweb.com/api/otp.php?authkey=246433AcgIcYWo5be18fb9&mobile=7353958937&message=Your%20OTP%20is%200808&sender=GRCJAR&otp=0808
    //         $ch = curl_init();
    //         curl_setopt($ch, CURLOPT_URL, $apiRoute);
    //         curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    //         $response = curl_exec($ch);
    ...
    // public function sendGlobalOtp($mobile, $otp)
    // { 
    //     $message = "Your OTP is #" . $otp . ". Please do not share this with anyone.";
    //     $this->sendGlobalSmsOtp($mobile, $message, $otp);
    // }

    // public function testsms($mobile)
    // {  
    //     $response = $this->getGlobal();
    //     $template = $this->getTemplatekey();
    //     $sender = $response['sms'];
    ...
    //       "message" => "Your order 123 is on the way -- PVC Bazar"
    //     );
    ...
    //     //$this->sendGlobalSms($datas);
    //     //$this->sendStatus($mobile, $id, $mode);
    //     //$this->sendOtp("91".$mobile, '1234');
    //     //$this->decodeJson($data, $order, $id);
    // }
